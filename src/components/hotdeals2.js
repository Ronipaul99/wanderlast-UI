...
class HotDealss extends Component {
    constructor(props) {
        super(props);
        this.state = {
            bookingForm: {
    ...
    getHotDeals = () => {
        ...
        axios.get(backendUrlPackage+"/hotDeals")
            .then((response) => {
                this.setState({ packages: response.data, show: false, errorMessage: null, spinner:false })
            })
            .catch((error) => {
                this.setState({ errorMessage: error.response.data.message,spinner:false, packages: null },()=>{
    ...
    calculateCharges = () => {
        ...
        bookingForm.tripend=finalCheckOutDate.getDate();
    ...
    loadBookingPage = (dealId) => {
        ...
        let day = checkOutDate.getDay();
        if(day<10){
          day="0"+day.toString()  
        }
        let month = checkOutDate.getMonth()+2;
        if(month<10){
        ...
        sessionStorage.setItem('totalAmount',this.state.totalCharges)
        this.setState({ show:true, bookingPage: true, showItinerary: false, dealId: dealId })
       //console.log("hiii"+this.state.totalCharges);
                    ...
                    <div className="card bg-light text-dark package-card" key={mypackage.destinationId}>
                        <div className="card-body row">
                            ...
                            <div className="col-md-3">
                                ...
                                <div className="text-center text-success"><h3>RS: {mypackage.chargesPerPerson}</h3></div><br /><br />
    ...
    render() {
        if(this.state.bookingPage === true) return ( <Redirect to={"/book/"+this.state.deal.destinationId}/>)
                        ...
                        (
                            <div>
                                <div style={{height:"150px"}}>
                                    <h1 className="text-success"> Get Flat 5% Discount on Every packages</h1>
                                    <h2 className="text-success">Hurry Up!!!.....</h2>
                                </div>
                                {this.displayPackages()}
                                {
                                    this.state.errorMessage ?
                                        (
                                            <div className="offset-md-1" style={{height:"400px", paddingTop:"100px"}}>
                                ...
                                {!this.state.totalCharges ?
                                    ...
                                    (
                                        <h4 className="text-success">
                                            Your trip ends on {this.state.checkOutDate.toDateString()} and
